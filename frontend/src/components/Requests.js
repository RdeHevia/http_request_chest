  ...
  return (
    <li>
      <section className="m-5">
        <h1 className="text-5xl font-bold">{request.method} {request.path}</h1>
        <dl>
          <dt className="text-white font-bold">Received on:</dt>
          <dd className="italic text-lg">{new Date(request.createdAt).toLocaleString("en-US", dateConfig)}</dd>
          <dt className="text-white font-bold">Protocol:</dt>
          <dd className="italic text-lg">{request.protocol.toUpperCase()}</dd>
          <dt className="text-white font-bold">From:</dt>
          <dd className="italic text-lg">{request.hostname}</dd>
          <dt className="text-white font-bold">IP address:</dt>
          <dd className="italic text-lg">{request.ip || "N/A"}</dd>
        </dl>
      </section>
      <section className="m-5">
        <h2 className="text-3xl font-bold">HEADERS</h2>
        <ul>
